...
class CategoriesService {
  ...
  Future<Resource<Category>> create(Category category, File file) async {
    try {
      ...
      Uri url = Uri.http(ApiConfig.API_ECOMMECE, '/categories');
  ...
  Future<Resource<Category>> update(int id, Category category) async {
    try {
      Uri url = Uri.http(ApiConfig.API_ECOMMECE, '/categories/$id');
      Map<String, String> headers = {
        "Content-Type": "application/json",
        "Authorization": await token
      };
  ...
  Future<Resource<Category>> updateImage(
    ...
    try {
      Uri url = Uri.http(ApiConfig.API_ECOMMECE, '/categories/upload/$id');
